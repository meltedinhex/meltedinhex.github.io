...
*Figure 1: The chain. A hacked WordPress site carries the injected loader in one of two
...
A one-paragraph summary so I don't re-explain what others have covered well. **ErrTraffic**
is a rented ClickFix kit (advertised by the actor "LenAI" on Exploit.IN since December 2025)
injected into compromised WordPress sites; it resolves its C2 from a Polygon smart contract
via **EtherHiding** rather than a hard-coded domain. The contract here,
...
## Four months of C2, read straight off the chain
...
## The ClickFix lure and the hand-off to the user
...
## The July payload, and what it steals
...
## Who is still infected: a July pivot
...
That pivot surfaced **64 sites loading multiple rotating hosts, 37 of them not in the
earlier public lists**. Checking a sample of them, **19+ were still serving a live injection
marker at the time of writing** — both the plaintext and base64+XOR variants. One honest
caveat: many sites that did *not* show the marker returned a generic parking/WAF page, so a
